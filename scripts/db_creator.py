...
with open(os.path.join(question_dir, "categories.yml"), "r") as f:
...
for category in data["trivia_categories"]:
    questions_path = os.path.join(
        question_dir, "questions_{}.yml".format(category["id"])
    )
    if os.path.exists(questions_path):
        session.add(Category(id=int(category["id"]), name=category["name"]))
        with open(questions_path) as q:
            questions = yaml.safe_load(q)
            for question in questions["questions"]:
                try:
                    difficulty, air_date, question_parsed = parse_question_string(
                        question["question"]
                    )
